...
In our analysis of the timber hardness data
...
mortality of batches of the flour beetle *Tribolium confusa* exposed to different doses of a pesticide.
...
link function and binomial distribution take account of the properties and constraints on the pattern of the mean and variance for binomial count
data.
...
However, one disadvantage of quasi-maximum likelihood is that because we have a
quasi-likelihood rather than a true likelihood we are not given a value for
the Akaike information criterion (AIC; although, as we will see later, some
...
wells <- read.delim("Binary_Wells.txt")
ggplot(wells,aes(dist,switch)) +geom_point() +
  geom_smooth() +
...
Gelman and Hill promote a rough rule of thumb for interpreting the slope of the logistic regression: the ‘divide by four rule’. Dividing the slope coefficient for the logistic regression slope by four will give us an approximate estimate for the maximum predicted effect on the response of a unit change in the predictor.
...
binomial, data= wells)
display(fit.5) # model numbering from Gelman and Hill
```
...
3. Interpretation of coefficients as odds ratios
